"""
AppX Uploader Bot v3 — main entry point.

Starts:
  1. SQLite database (init schema)
  2. aiohttp web API server (optional)
...
from config.settings import (
    API_ENABLED, BOT_TOKEN, DB_PATH, TEMP_DIR,
)
...
from bot.handlers import (
    ...
    _idx_cb, _keys_save, _login_email, _login_pass,
    _proxy_save,
    callback_router,
    ...
    cmd_start, cmd_status, cmd_support, cmd_svg,
...
USER_COMMANDS = [
    ...
    BotCommand("check",    "Validate a URL"),
    BotCommand("info",     "URL/job information"),
    ...
    BotCommand("svg",      "Download welcome card SVG"),
]

ADMIN_COMMANDS = [
    BotCommand("admin",       "Admin control panel"),
    BotCommand("stats",       "Global bot statistics"),
    ...
    BotCommand("debug",       "Debug information"),
]
...
def build_app(db: Database) -> Application:
    ...
    # Register conversations first (they take priority)
    ...
    for name, fn in {
        ...
        "clear":    cmd_clear,    "history":  cmd_history,
        "profile":  cmd_profile,  "mystats":  cmd_mystats,
        "settings": cmd_settings, "notify":   cmd_notify,
        "language": cmd_language, "check":    cmd_check,
        "info":     cmd_info,     "ping":     cmd_ping,
        "about":    cmd_about,    "version":  cmd_version,
        "support":  cmd_support,  "export":   cmd_export,
        "quota":    cmd_quota,    "svg":      cmd_svg,
    }.items():
...
async def main() -> None:
    ...
    # Load saved config overrides into runtime settings
    from config import settings as cfg
    proxy = await db.get_config("http_proxy", "")
    if proxy: cfg.HTTP_PROXY = proxy
    ...
    try:
        ...
        logger.info("Bot commands registered (%d user, %d admin)",
                    len(USER_COMMANDS), len(ADMIN_COMMANDS))
    except Exception as e:
        logger.warning("set_my_commands failed: %s", e)

    logger.info("🚀 %s starting…", cfg.BOT_NAME)
