"""All inline keyboard layouts — v4.3 (AppX V2 bypass fix)."""
...
def channel_bypass_kb(bypass_url: str, bot_username: str = "") -> Optional[M]:
    ...
    For AppX PDF / standard content: shows clickable URL button.
    For AppX V2 video: bypass_url is empty string — only "Get via Bot" shows.
    """
    rows = []
    if bypass_url:
...
def channel_file_kb(bypass_url: str = "", bot_username: str = "") -> Optional[M]:
    """
    Keyboard attached to a file message in a channel.

    bypass_url = "" for AppX V2 video (no public URL exists).
    Only "Get More Files" bot link shows in that case.
    """
    rows = []
    if bypass_url:
...
def v2_download_kb(bot_username: str = "") -> Optional[M]:
    """
    Keyboard for V2 encrypted video bypass messages.
    Explains that no browser link is possible; directs users to the bot.
    """
    rows = [
        [B("🔐 What is V2 encrypted?", callback_data="v2_info")],
    ]
    if bot_username:
