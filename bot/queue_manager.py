"""
Job queue engine — v3 FIXED.
Changes: title passed through to downloader so filenames use video/lecture names.
"""
...
async def cancel_job(jid: str, db: Database) -> bool:
    ...
    if cev:
        cev.set()
        _pauses.get(jid, asyncio.Event()).clear()
        return True
    return False
...
async def _run(
    ...
    for idx in range(start, len(entries)):
        ...
        if pev.is_set():
            ...
            try:
                await bot.edit_message_text(
                    chat_id=chat_id, message_id=pmid,
                    text=prog.render(jid) + "\n\n⏸ <b>Paused.</b> Tap ▶️ Resume to continue.",
                    parse_mode="HTML",
        ...
        url        = url.strip()
        prog.current  = idx
        prog.filename = (title or os.path.basename(url.split("?")[0]))[:40]
    ...
    if user and user.get("notify_done") and fstatus == "completed":
        try:
            await bot.send_message(
                chat_id=uid,
                text=f"🔔 Job <code>{jid[:8]}</code> complete! ✅ {prog.completed} files uploaded.",
                parse_mode="HTML",
            )
        except Exception:
            pass
    ...
    try:
        ...
        try:
            await bot.send_message(chat_id=chat_id, text=ftxt, parse_mode="HTML", reply_markup=kb)
        except Exception:
            pass
