"""
Job queue engine — v4.4  (AppX V2 bypass full fix)

KEY CHANGES vs v4.3:
  1. Channel bypass message for V2 is now SHORTER and cleaner.
     Shows a "📥 Bypass & Get File" button that deep-links to the bot
     with the job ID so users can explicitly request the file.
  2. V2 download now passes `cookie` all the way to _download_appx_v2
     so plain HTTP with Bearer token works correctly.
  3. File IS sent to the channel automatically after download (Step 4) —
     no user action needed if the bot has the right token.
"""
...
async def _run(
    ...
    for idx in range(start, len(entries)):
        ...
        # For V2: post a CLEAN message with bypass button.
        # The file will be sent to the channel AUTOMATICALLY after download (Step 4).
        # Users clicking the bypass button go to the bot for on-demand re-download.
        if CHANNEL_IDS:
            ...
            if is_v2_video:
                bypass_text = (
                    ...
                    f"⬇️ <b>Bot is downloading now...</b>\n"
                    f"File will be sent here automatically. "
                    f"Tap ↓ button if it doesn't arrive."
                )
                # Show bypass + bot button — deep-link includes job info
                bypass_kb = channel_bypass_kb(
                    bypass_url="",          # no public browser URL
                    bot_username=BOT_USERNAME,
                    jid=jid[:8],
                    is_v2=True,
                )
            else:
                # Standard AppX — try to resolve URL first for the bypass link
                resolved_url = url
                try:
                    r_url, _r_h, _r_kind = await dl.resolve_url(
                ...
                bypass_kb = channel_bypass_kb(
                    resolved_url, BOT_USERNAME, jid=jid[:8], is_v2=False,
                )
            ...
            for ch_id in CHANNEL_IDS:
                try:
                    ...
                    logger.info("[%s] Bypass msg → channel %s (v2=%s)",
        ...
        try:
            for att in range(1, MAX_RETRIES + 1):
                if cev.is_set(): break
                try:
                    prog.phase = "downloading" if att == 1 else f"retry {att}"
        ...
        if not ok:
            ...
            await _edit()
            # Notify channel the download failed
            if CHANNEL_IDS and is_v2_video:
                fail_note = (
                    f"❌ <b>Download Failed</b>\n\n"
                    f"📄 <b>{display_name[:150]}</b>\n"
                    f"🆔 Job: <code>{jid[:8]}</code>\n\n"
                    f"<i>{errmsg[:200]}</i>\n\n"
                    f"💡 If token expired: set a fresh cookie with /cookie"
                )
                for ch_id in CHANNEL_IDS:
                    try:
                        await bot.send_message(
                            chat_id=ch_id, text=fail_note,
                            parse_mode="HTML",
                            disable_web_page_preview=True,
                        )
                    except Exception:
        ...
        try:
            ...
            # Send to the requesting user (private)
            await _send(bot, chat_id, path, fname, mime, title, idx + 1, len(entries))

            # Send the ACTUAL FILE to each channel
            # (this is the "direct file in channel" the user asked for)
            if CHANNEL_IDS:
                file_kb = channel_file_kb("", BOT_USERNAME)
                for ch_id in CHANNEL_IDS:
                    try:
                        await _send(
                        ...
                        logger.warning("[%s] Channel %s file upload: %s", jid[:8], ch_id, ch_e)
