"""Job queue: sequential processing with pause/resume/cancel via asyncio Events."""
...
async def start_job(bot, db: Database, uid: int, chat_id: int,
                     urls: List[str], start: int, pmid: int,
                     source_name: str = "") -> str:
    jid = new_jid()
    ...
    t = asyncio.create_task(
        _run(bot, db, jid, uid, chat_id, urls, start, pmid, pev, cev),
        name=f"job-{jid[:8]}"
    )
    _tasks[jid] = t
    logger.info("Job %s started: %d links from idx %d", jid, len(urls), start)
    return jid


async def pause_job(jid: str) -> bool:
    ev = _pauses.get(jid)
    if ev and not ev.is_set(): ev.set(); return True
    return False


async def resume_in_place(jid: str) -> bool:
    ev = _pauses.get(jid)
    if ev and ev.is_set(): ev.clear(); return True
    return False


async def cancel_job(jid: str, db: Database) -> bool:
    cev = _cancels.get(jid)
    if cev: cev.set(); _pauses.get(jid, asyncio.Event()).clear(); return True
    return False


async def resume_from_db(bot, db: Database, jid: str, uid: int,
                          chat_id: int, urls: List[str], pmid: int) -> bool:
    job = await db.get_job(jid)
    if not job or job["status"] not in ("paused", "running"): return False
    old = _tasks.get(jid)
    ...
    t = asyncio.create_task(
        _run(bot, db, jid, uid, chat_id, urls, job["current_index"], pmid, pev, cev),
        name=f"job-{jid[:8]}-resume"
    )
    _tasks[jid] = t
    return True
...
async def _run(bot, db: Database, jid: str, uid: int, chat_id: int,
               urls: List[str], start: int, pmid: int,
               pev: asyncio.Event, cev: asyncio.Event) -> None:
    dl     = get_downloader()
    prog   = JobProgress(total=len(urls), start=start)
    ...
    async def _edit(extra: str = "") -> None:
        ...
        if now - last_ed < EDIT_GAP: return
        last_ed = now
        try:
    ...
    for idx in range(start, len(urls)):
        ...
        if pev.is_set():
            ...
            try:
                await bot.edit_message_text(
                    chat_id=chat_id, message_id=pmid,
                    text=prog.render(jid) + "\n\n⏸ <b>Paused.</b> Press ▶️ Resume to continue.",
                    parse_mode="HTML",
        ...
        url = urls[idx].strip()
        prog.current  = idx
        prog.filename = os.path.basename(url.split("?")[0])[:40]
        ...
        for att in range(1, MAX_RETRIES + 1):
            ...
            try:
                ...
                path, fname, mime = await dl.download(
                    url, dest_dir=job_tmp, progress_cb=_cb,
                    job_id=jid, cookie=cookie, drm_keys=drm_keys,
                )
                prog.filename = fname
                ok = True
                break
                ...
                errmsg = str(e)[:200]
                logger.warning("Attempt %d/%d — %s: %s", att, MAX_RETRIES, url[:60], e)
                if att < MAX_RETRIES:
                    await asyncio.sleep(4 * att)
        ...
        try:
            ...
            await _send(bot, chat_id, path, fname, mime, idx+1, len(urls))
            ...
            uperr = str(e)[:200]
            logger.error("Upload failed %s: %s", fname, e)
    ...
    if cev.is_set():
        fstatus = "cancelled"
        ftxt = (f"⏹ <b>Job Cancelled</b>\n\n"
                f"✅ Completed : <b>{prog.completed}</b>\n"
                f"❌ Failed    : <b>{prog.failed}</b>\n"
                f"⏭ Remaining : <b>{len(urls)-prog.current}</b>\n"
                f"🆔 <code>{jid[:8]}</code>")
    else:
        fstatus = "completed"
        ftxt = (f"🎉 <b>Job Complete!</b>\n\n"
    ...
    if user and user.get("notify_done") and fstatus == "completed":
        try:
            await bot.send_message(chat_id=uid,
                text=f"🔔 Your job <code>{jid[:8]}</code> is complete!\n"
                     f"✅ {prog.completed} files uploaded.", parse_mode="HTML")
        except Exception: pass
    ...
    try:
        ...
        try: await bot.send_message(chat_id=chat_id, text=ftxt, parse_mode="HTML", reply_markup=kb)
        except Exception: pass

    _tasks.pop(jid, None); _pauses.pop(jid, None); _cancels.pop(jid, None)


async def _send(bot, chat_id: int, path: str, fname: str,
                 mime: str, idx: int, total: int) -> None:
    cap = f"📄 <b>{fname}</b>\n📦 File {idx}/{total}"
    mt  = mime.lower()
    with open(path, "rb") as fh:
        if mt.startswith("video/"):
            ...
            await bot.send_audio(chat_id=chat_id, audio=fh, caption=cap, parse_mode="HTML")
        elif mt.startswith("image/"):
            await bot.send_photo(chat_id=chat_id, photo=fh, caption=cap, parse_mode="HTML")
        else:
