...
# ── /announce  — send to all target channels ─────────────────────────────────
async def cmd_announce(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    for ch_id in CHANNEL_IDS:
        try:
            await context.bot.send_message(
                chat_id=ch_id, text=msg, parse_mode=ParseMode.HTML
            )
            sent += 1
        except Exception as e:
            logger.warning("Announce to %s failed: %s", ch_id, e)
            fail += 1
    result = f"✅ Announcement sent to {sent} channel(s)."
    if fail:
        result += f"\n⚠️ Failed for {fail} channel(s) — check logs."
    await update.message.reply_text(result, reply_markup=back_admin())
