...
async def api_bypass(request: web.Request) -> web.Response:
    ...
    try:
        ...
        if resolved == url:
            ...
            strategy = "fallback"
        elif any(h in resolved for h in (
            "appxcdn", "d1bsb8xfl4oazp", "appxcontent", "appxlectures",
        )):
            strategy = "bypass-v2-cdn"
        elif "token=" in resolved or "auth=" in resolved:
            strategy = "bypass-v2-token-param"
        elif any(h in resolved for h in ("static-db", "cdn.appx", "media.appx")):
            strategy = "bypass-v1-cdn-rotation"
        elif "cloudfront" in resolved.lower() or "amazonaws" in resolved.lower():
            strategy = "bypass-v1-signed-url"
        else:
            strategy = "bypass-resolved"
